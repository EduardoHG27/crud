...
$no_cuenta =$res[0]->no_cu;
$this->db->query('select DESC_CONCEPTO_DETFAC,IMP_VENCIDO_DETFAC,IMP_MES_DETFAC,IMP_TOT_DESFAC from dat_fac_detalle WHERE no_recibo_detfac= "' . $id . ' "');
...
$pdf->Cell(60,4,'No. Ticket: '.$id,0,1,'');
$pdf->Cell(60,4,'No. Cuenta: '.$no_cuenta,0,1,'');
...
for ($i = 1; $i <= $num; $i++) {
    ...
    $pdf->MultiCell(30,4,$res_fac[$numero]->DESC_CONCEPTO_DETFAC,0,'L'); 
...
$pdf->SetFont('Helvetica', '', 7);
$pdf->SetFont('Helvetica', '', 7);
$pdf->MultiCell(30,4,'Primer Pago',0,'L'); 
